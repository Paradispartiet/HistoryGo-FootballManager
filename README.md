...
- **Neste handling (Oversikt):** en ny stripe øverst på Oversikt viser hvor i uka treneren er (uke + Club Week-fase) og **hva han bør gjøre nå** — én tydelig primærhandling (stor, invertert knapp) + opptil to sekundære «chips». Handlingene utledes av `computeManagerNextActions(teamFit)` i `src/app.js` fra **eksisterende state**: pågående kampsesjon, roster readiness (15-kravet), tomme/feilbrukte/dupliserte plasser i startelleveren, Club Week-matchday-porten, treningsvalg for uka, kampklar-status, uleste innbokstråder, sist spilte kamp og mini-sesongstatus. Prioriteringen er fast (mest til minst presserende) og hver handling rerouter til riktig fane/plass via eksisterende handlere (`activateTab`, `selectSlotDecision`, `startMiniSeason`, `advanceClubWeekPhaseAction`). Verbene er spillbare: «Fullfør startelleveren», «Velg roller», «Velg treningsprogram», «Les innboksen», «Spill kamp», «Se kamprapporten», «Start prøveperiode», «Gå til neste fase/uke». Ingen ny motor — kun prioritert presentasjon.
...
- **Validering:** ingen nye motorer, så ingen ny sim. `npm run check:dom-ids` og `npm run audit:flow` dekker at de nye DOM-id-ene finnes og at spilløkka fortsatt henger sammen; hele den vanlige suiten (`typecheck`, `build`, alle `audit:*`/`sim:*`) er grønn.
