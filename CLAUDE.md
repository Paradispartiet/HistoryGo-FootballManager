...
- `classHeight` (formerly `overall`; players schema **v4**) is the player's **level** — how far he actually got in his career — and sets the *ceiling* of his skill profile. It is an input, never a score, and never decides a match alone. The band is **75–99**, defined by `classTiers` in the players file (`bredde` → `all_time`); read the band from that table, never hardcode it. It used to be 85–100 with 204 of 367 players on exactly 87, which described the generator's tier rather than a career. Each player also carries `classSource`: `belagt` (162 — the career is publicly known) or `utledet` (205 — the source doesn't reach a career claim, so the level is derived from the signal already in the data). 167 players share level 79; that is the squad tier the generator never distinguished, and their profiles differ by **shape**, not level.
