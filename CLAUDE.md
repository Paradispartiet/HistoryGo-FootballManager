...
- `football-historical-opponent-profiles.js` — opponents are historical style-teams (taktiske arketyper) used as learning opponents, not generic bots. **League clubs carry an `archetypeId` too** (`LEAGUE_OPPONENT_PROFILES`): the club owns name/ground/strength, the archetype owns the football. Before that, the lookup searched for the *club id* among the five generic profiles where it could never match, so all 14 league rounds silently used the same profile. `sim:league-season` now walks a whole season and asserts 7 distinct schools. See `docs/liga-arketyper.md`.
