...
check("kontoret ligger før dybden", html.indexOf('id="officeCommand"') > -1 && html.indexOf('id="officeCommand"') < html.indexOf('id="officeDepth"'));
check("klubbukas detaljer er foldet", /<details class="office-depth"/.test(html) && /Vis klubbukas detaljer og signaler/.test(html));
check("kontoret lager ingen ny primærhandling", !/next-action-primary/.test(office) && /autoritative «Neste handling»/.test(office));
check("kontoret viser fire operative statuser", /office-status-grid/.test(office) && /"lineup"/.test(office) && /"training"/.test(office) && /"inbox"/.test(office) && /"readiness"/.test(office));
...
check("assistentråden har aktiv fokussak", /id="inboxFocusTitle"/.test(html) && /id="inboxThreadList"/.test(html) && /inbox-focus-panel/.test(css));
check("assistentråden har inline beslutningsstøtte", /inbox-inline-stats/.test(html) && /id="inboxSignalUnread"/.test(html) && /id="inboxSignalReplies"/.test(html));
check("assistentråden har kø", /id="inboxQueueList"/.test(html) && /queueCandidates/.test(app) && /selectedCandidate/.test(app));
