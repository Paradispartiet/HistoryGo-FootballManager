...
// Overgangsmarked v2. Alle beløp er HGFM-spillverdier, aldri historiske
// overgangssummer/markedsverdier. classHeight/Overall brukes ikke.
...
const integer = (value, fallback = 0) => Number.isFinite(Math.trunc(Number(value))) ? Math.trunc(Number(value)) : fallback;
const ids = (value) => Array.isArray(value) ? [...new Set(value.map((entry) => String(entry || "").trim()).filter(Boolean))] : [];
...
function normalizeOffer(value) {
  ...
  return {
    ...
    playerId: String(value.playerId), bidderClubId: String(value.bidderClubId || ""),
    bidderClubName: String(value.bidderClubName || "Annen klubb"), amount: Math.max(0, integer(value.amount)),
    windowKey: String(value.windowKey), seasonNumber: Math.max(1, integer(value.seasonNumber, 1)),
    round: Math.max(1, integer(value.round, 1))
  };
}

function normalizeHistory(value) {
  return Array.isArray(value) ? value.filter(isObject).slice(-40).map((entry) => ({
    id: String(entry.id || `${entry.type || "event"}:${entry.windowKey || "window"}:${entry.playerId || "player"}`),
    type: String(entry.type || "event"), playerId: entry.playerId ? String(entry.playerId) : null,
    ...
    amount: Math.max(0, integer(entry.amount)), windowKey: entry.windowKey ? String(entry.windowKey) : null,
    seasonNumber: Math.max(1, integer(entry.seasonNumber, 1)), round: Math.max(1, integer(entry.round, 1))
  })) : [];
}
...
function completeRecruitmentSaleInMerits(merits, playerId, amount, { tierId = null, seasonNumber = 1 } = {}) {
  ...
  const saleAmount = Math.max(0, integer(amount));
  ...
  const contracts = { ...economy.contracts }; delete contracts[id];
  const ledger = [...economy.ledger, {
    id: `transfer_sale-${Math.max(1, integer(seasonNumber, 1))}-${id}-${saleAmount}`,
    type: "transfer_sale", season: Math.max(1, integer(seasonNumber, 1)), playerId: id,
    amount: saleAmount, label: "HGFM-spillersalg"
  }].slice(-40);
  return {
    changed: true,
    merits: {
...
export function transferWindowForSeason(season) {
  ...
  if (season?.status && season.status !== "active") return {
    open: false, key: `s${seasonNumber}:closed`, label: "Vindu stengt", phase: "closed", seasonNumber, currentRound, totalRounds,
    nextLabel: "Neste HGFM-vindu åpner ved starten av neste sesong."
  };
  if (currentRound <= OPENING_WINDOW_ROUNDS) return {
    open: true, key: `s${seasonNumber}:opening`, label: `HGFM-vindu · runde 1–${OPENING_WINDOW_ROUNDS}`,
    phase: "opening", seasonNumber, currentRound, totalRounds
  };
  if (currentRound >= midStart && currentRound < midStart + MIDSEASON_WINDOW_ROUNDS) return {
    open: true, key: `s${seasonNumber}:midseason`, label: `HGFM-vindu · runde ${midStart}–${midStart + MIDSEASON_WINDOW_ROUNDS - 1}`,
    phase: "midseason", seasonNumber, currentRound, totalRounds
  };
  return {
    open: false, key: `s${seasonNumber}:closed:r${currentRound}`, label: "Vindu stengt", phase: "closed", seasonNumber, currentRound, totalRounds,
    nextLabel: currentRound < midStart ? `Neste HGFM-vindu åpner i runde ${midStart}.` : "Neste HGFM-vindu åpner ved starten av neste sesong."
  };
}
...
export function normalizeTransferMarket(input, season = null) {
  ...
  const offers = {};
  if (isObject(src.offers)) Object.entries(src.offers).forEach(([playerId, raw]) => {
    const offer = normalizeOffer(raw); if (offer) offers[String(playerId)] = offer;
  });
  return {
    version: TRANSFER_MARKET_VERSION, listedPlayerIds: ids(src.listedPlayerIds), offers,
    closedOfferKeys: ids(src.closedOfferKeys), history: normalizeHistory(src.history),
...
export function transferOfferUnits(contract) {
  return 8 + Math.max(1, integer(contract?.remainingSeasons, 1)) * 2 + Math.max(0, integer(contract?.wageUnits));
}
...
export function generateTransferOfferForListedPlayer(merits, playerId, season) {
  ...
  if (market.closedOfferKeys.includes(`${window.key}:${id}`)) return { changed: false, merits: base, market, offer: null };
  ...
  if (!contract || !clubs.length) return { changed: false, merits: base, market, offer: null };
  const bidder = clubs[hash(`${window.key}:${id}`) % clubs.length];
  const offer = {
    id: `${window.key}:${id}:${bidder.id}`, playerId: id, bidderClubId: String(bidder.id),
    bidderClubName: String(bidder.name || bidder.shortName || bidder.id), amount: transferOfferUnits(contract),
    windowKey: window.key, seasonNumber: window.seasonNumber, round: window.currentRound
  };
...
export function listRecruitedPlayerForTransfer(merits, playerId, season) {
  ...
  const listedMarket = { ...market, listedPlayerIds: [...market.listedPlayerIds, id], lastSeenWindowKey: window.key };
  const listedMerits = { ...base, transferMarket: listedMarket };
  const generated = generateTransferOfferForListedPlayer(listedMerits, id, season);
  return generated.changed
    ? { ...generated, changed: true, reason: `Spilleren er lagt ut. ${generated.offer.bidderClubName} har sendt et bud.` }
    : { changed: true, reason: "Spilleren er lagt ut for bud i dette HGFM-vinduet.", market: listedMarket, merits: listedMerits };
}

export function withdrawRecruitedPlayerFromTransfer(merits, playerId, season) {
  const base = isObject(merits) ? merits : {}; const id = String(playerId || "").trim();
  const market = normalizeTransferMarket(base.transferMarket, season);
  ...
  const offers = { ...market.offers }; delete offers[id];
...
export function reconcileTransferMarketInMerits(merits, season) {
  ...
  const offers = Object.fromEntries(Object.entries(market.offers).filter(([, offer]) => window.open && offer.windowKey === window.key));
  ...
  if (window.open) for (const playerId of market.listedPlayerIds) {
    const generated = generateTransferOfferForListedPlayer(nextMerits, playerId, season);
    if (generated.changed) { changed = true; nextMerits = generated.merits; market = generated.market; }
  }
...
export function rejectTransferOfferInMerits(merits, playerId, season) {
  const base = isObject(merits) ? merits : {}; const id = String(playerId || "").trim();
  const market = normalizeTransferMarket(base.transferMarket, season); const offer = market.offers[id];
  if (!offer) return { changed: false, reason: "Ingen aktivt bud å avslå.", merits: base, market };
  const offers = { ...market.offers }; delete offers[id];
  const next = {
    ...market, offers, closedOfferKeys: ids([...market.closedOfferKeys, `${offer.windowKey}:${id}`]),
    history: [...market.history, { ...offer, type: "rejected" }].slice(-40)
  };
...
export function acceptTransferOfferInMerits(merits, playerId, season) {
  ...
  const sale = completeRecruitmentSaleInMerits(base, id, offer.amount, { tierId: season?.tier?.id || season?.competition?.tierId, seasonNumber: season?.seasonNumber });
  if (!sale.changed) return { changed: false, reason: sale.reason, merits: base, market };
  const saleMarket = normalizeTransferMarket(sale.merits.transferMarket, season); const offers = { ...saleMarket.offers }; delete offers[id];
  const next = {
    ...saleMarket, listedPlayerIds: saleMarket.listedPlayerIds.filter((entry) => entry !== id), offers,
    closedOfferKeys: ids([...saleMarket.closedOfferKeys, `${offer.windowKey}:${id}`]),
    history: [...saleMarket.history, { ...offer, type: "sold" }].slice(-40)
  };
  return {
    changed: true, reason: `${offer.bidderClubName} kjøper spilleren for ${offer.amount} HGFM-klubbmidler.`, offer, market: next,
