...
function ensureWorkspace() {
  ...
  workspace = node("div", "manager-player-workspace"); workspace.id = WORKSPACE_ID; workspace.innerHTML = `
    <header class="manager-player-workspace-head"><div><p class="eyebrow">Lag · Tropp</p><h2>Spillerliste</h2><p class="muted-text">Sammenlign troppen her. Åpne en spiller for detaljene; laguttak endres bare med en eksplisitt Velg-knapp på Oppstilling.</p></div><strong id="managerRosterCount" class="manager-roster-count">0 spillere</strong></header>
...
function createPlayerLink(row) { const button = node("button", "manager-roster-player-link"); button.type = "button"; button.dataset.playerProfileId = row.id; button.append(node("strong", "", row.name), node("small", "", row.nationality || "")); button.addEventListener("click", () => openManagerPlayerProfile(row.id)); return button; }
function renderRoster(rows = liveRows(), { force = false } = {}) {
  ...
  const body = workspace.querySelector("#managerRosterBody"); if (!body) return; const fragment = document.createDocumentFragment(); visible.forEach((row) => {
    ...
    [playerCell, positionCell, roleCell, statusCell, fitCell, formCell, node("td", "manager-roster-number", row.appearances), node("td", "manager-roster-number", row.goals), node("td", "manager-roster-number", row.assists), node("td", "manager-roster-training", trainingLabel)].forEach((cell) => tr.append(cell)); tr.addEventListener("dblclick", () => openManagerPlayerProfile(row.id)); fragment.append(tr);
...
function renderCurrentPanel(data) { const { row, player, recruitable } = data; const section = node("section", "manager-player-current-panel"); section.append(node("h3", "", "Akkurat nå")); const metrics = node("div", "manager-player-current-grid"); [["Kampklarhet", row.status.label, describeCondition(row.condition)], ["Form", row.form.text, `${row.appearances} kamper denne sesongen`], ["Taktisk fit", row.fit.label, selectedTacticId() ? "Mot valgt kampplan" : "Velg kampplan for vurdering"], ["Rekruttering", recruitable ? "Tilgjengelig" : "Ikke tilgjengelig ennå", recruitable ? "Spilleren er i din tilgjengelige spillerpool." : "Profilen kan undersøkes, men spilleren er ikke låst opp som kandidat."], ["Individuell trening", row.training ? runtime.trainingById.get(row.training.trackId)?.name || formatToken(row.training.trackId || row.training.roleId) : "Ingen", row.training?.roleId ? `Rolle: ${formatToken(row.training.roleId)}` : "Ingen aktiv oppfølging"]].forEach(([label, value, detail]) => { const article = node("article"); article.append(node("span", "", label), node("strong", "", value), node("small", "", detail)); metrics.append(article); }); section.append(metrics); if (player.warningWhenMisused) { const warning = node("div", "manager-player-misuse-warning"); warning.append(node("span", "", "Misbruksvarsel"), node("strong", "", player.warningWhenMisused)); section.append(warning); } return section; }
