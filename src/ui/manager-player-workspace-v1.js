...
function historyGoPlaceIds() {
  ...
  if (Array.isArray(visited)) {
    ...
    Object.entries(visited).forEach(([id, value]) => {
      if (value) ids.add(String(id));
    });
  }
  const groundhopper = readStorage(STORAGE.groundhopper, null);
  const places = groundhopper?.visited_groundhopper_places
...
function buildStarterSquad(players, candidateIds, limit = 15) {
  const candidates = asArray(players)
    .filter((player) => candidateIds.has(String(player.id)))
    .sort((a, b) => (Number(a.classHeight) || 0) - (Number(b.classHeight) || 0)
...
function resolveUnlockedPlayerIds(players, unlockData) {
  ...
  asArray(unlockData?.placeUnlocks).forEach((place) => {
    asArray(place?.unlocks).forEach((unlock) => {
      ...
      candidateIds.add(id);
      if (placeIds.has(String(place.placeId))) unlocked.add(id);
    });
  });

  if (!unlocked.size) buildStarterSquad(players, candidateIds).forEach((id) => unlocked.add(id));
  return unlocked;
}
...
export function createRosterViewModel({
  ...
  return asArray(players).filter((player) => unlocked.has(String(player.id))).map((player) => {
    ...
    return {
      ...
      role: bestRole(player, rolesById, roleFamiliarity),
      status: conditionStatus(condition),
      form: formStatus(condition),
      fit: tacticFit(player, tacticId),
      appearances: Number(stat.appearances) || 0,
      goals: Number(stat.goals) || 0,
      assists: Number(stat.assists) || 0,
      minutes: Number(stat.minutes) || 0,
      condition,
      training: trainingMap.get(String(player.id)) || null,
...
async function loadRuntime() {
  ...
  runtimePromise = Promise.all([
    ...
    runtime = {
      ...
      catalogue,
      attributeIndex,
      placesById: new Map(asArray(unlocks?.placeUnlocks).map((place) => [place.placeId, place.placeName || formatToken(place.placeId)])),
      trainingById: new Map(asArray(trainingData?.tracks).map((track) => [track.id, track]))
    };
    return runtime;
  });
...
function liveRows({ includePlayerId = null } = {}) {
  ...
  if (includePlayerId) unlocked.add(String(includePlayerId));
...
function ensureCompactStatus() {
  ...
  const oldCommand = document.getElementById("squadTacticsCommandPanel");
  if (oldCommand && !oldCommand.classList.contains("is-replaced-by-compact-status")) oldCommand.classList.add("is-replaced-by-compact-status");
  return rail;
}

function parseRatio(id, fallbackTotal) {
  const match = text(document.getElementById(id)?.textContent).match(/(\d+)\s*\/\s*(\d+)/);
  return { current: match ? Number(match[1]) : 0, total: match ? Number(match[2]) : fallbackTotal };
}
...
function renderCompactStatus(rows = liveRows()) {
  ...
  const unavailable = rows.filter((row) => ["injured", "tired"].includes(row.status.id)).length;
  const formation = text(document.querySelector("#formationSelect")?.selectedOptions?.[0]?.textContent, "Ingen formasjon");
  const labels = [
    `Tropp ${rows.length}`,
...
function ensureWorkspace() {
  ...
  workspace.innerHTML = `
    ...
    <div class="manager-roster-table-wrap">
      <table class="manager-roster-table">
        <thead><tr><th>Spiller</th><th>Pos</th><th>Rolle</th><th>Status</th><th>Fit</th><th>Form</th><th>Kamper</th><th>Mål</th><th>Målgivende</th><th>Trening</th></tr></thead>
  ...
  const positionSelect = workspace.querySelector("#managerRosterPosition");
  Object.keys(POSITION_ORDER).sort((a, b) => POSITION_ORDER[a] - POSITION_ORDER[b]).forEach((position) => {
    const option = node("option", "", position);
    option.value = position;
    positionSelect?.append(option);
  });
  workspace.querySelector("#managerRosterTools")?.addEventListener("input", scheduleRosterRender);
  workspace.querySelector("#managerRosterTools")?.addEventListener("change", scheduleRosterRender);
  return workspace;
}

function rosterControls(workspace) {
  return {
...
function rosterSignature(rows, controls) {
  return JSON.stringify({
    controls,
    rows: rows.map((row) => [row.id, row.status.id, row.fit.id, row.form.label, row.appearances, row.goals, row.assists, row.training?.trackId || row.training?.roleId || ""])
  });
}

function createPlayerLink(row) {
  const button = document.createElement("button");
  button.type = "button";
  button.className = "manager-roster-player-link";
  button.dataset.playerProfileId = row.id;
  button.append(node("strong", "", row.name), node("small", "", row.nationality || ""));
...
function renderRoster(rows = liveRows(), { force = false } = {}) {
  ...
  if (!workspace) return;
  const controls = rosterControls(workspace);
  const visible = filterRosterRows(rows, controls);
  const signature = rosterSignature(visible, controls);
  ...
  visible.forEach((row) => {
    ...
    playerCell.append(createPlayerLink(row));
    ...
    roleCell.append(node("span", "", row.role.name), node("small", "", row.role.familiarity ? `${row.role.familiarity}% · ${row.role.familiarityLabel}` : row.role.familiarityLabel));
...
function scheduleRosterRender() {
  cancelAnimationFrame(rosterFrame);
  rosterFrame = requestAnimationFrame(() => {
    rosterFrame = 0;
    if (!runtime) return;
    const rows = liveRows();
...
function profileData(playerId, { allowLocked = false } = {}) {
  ...
  const rows = liveRows({ includePlayerId: allowLocked ? playerId : null });
  ...
  return {
    row,
    player: row.player,
    attributes: runtime.attributeIndex?.profiles?.[playerId] || null,
    recruitable: unlocked.has(String(playerId))
  };
}
...
function renderMiniPitch(player) {
  ...
  ].forEach(([positions, kind, label]) => positions.forEach((position) => {
...
function attributeGroups(profile) {
  ...
  Object.entries(profile.values || {}).forEach(([id, value]) => {
    ...
    groups[category]?.push({ name: meta?.name || formatToken(id), value, source: profile.provenance?.[id] || "utledet" });
  });
...
function renderAttributes(profile) {
  ...
  heading.append(node("h3", "", "Ferdighetsprofil"), node("span", "", "1–20 · ingen overall"));
  host.append(heading);
...
function renderRolePanel(player) {
  const store = readStorage(STORAGE.merits, {})?.roleFamiliarity || {};
  ...
  const roles = asArray(player.preferredRoles);
  if (!roles.length) list.append(node("p", "muted-text", "Ingen foretrukne roller er registrert."));
  roles.forEach((roleId) => {
    const familiarity = getRoleFamiliarity(store, player.id, roleId);
    const description = describeRoleFamiliarity(familiarity);
    const card = node("article", "manager-player-role-card");
    card.append(
      node("strong", "", roleMap.get(roleId)?.name || formatToken(roleId)),
      node("span", "", `${familiarity}% · ${description.label}`),
      node("small", "", description.hint)
    );
    list.append(card);
  });
  section.append(list);
...
function renderCurrentPanel(data) {
  const { row, player, recruitable } = data;
  ...
  [
    ...
    ["Rekruttering", recruitable ? "Tilgjengelig" : "Ikke tilgjengelig ennå", recruitable ? "Spilleren er i din tilgjengelige spillerpool." : "Profilen kan undersøkes, men spilleren er ikke låst opp som kandidat."],
...
function profileTabContent(data, tab) {
  ...
  if (tab === "training") {
    ...
    if (row.training) {
      trainingBox.append(node("strong", "", runtime.trainingById.get(row.training.trackId)?.name || formatToken(row.training.trackId || row.training.roleId)));
      if (row.training.roleId) trainingBox.append(node("p", "muted-text", `Rollefokus: ${formatToken(row.training.roleId)}`));
    } else {
  ...
  if (tab === "history") {
    ...
    source.append(node("h3", "", "History Go-opprinnelse"), node("p", "", places.length ? places.join(" · ") : "Ingen stedskilde registrert."));
  ...
  [["Kamper", row.appearances], ["Minutter", row.minutes], ["Mål", row.goals], ["Målgivende", row.assists]].forEach(([label, value]) => {
    const article = node("article");
    article.append(node("span", "", label), node("strong", "", value));
    season.append(article);
  });
...
function renderProfile(playerId, { allowLocked = false } = {}) {
  const data = profileData(playerId, { allowLocked });
  ...
  const close = node("button", "manager-player-profile-close", "×");
  close.type = "button";
  ...
  right.append(renderCurrentPanel(data), tokenList("Styrker", player.strengths, "is-positive"), tokenList("Trenger rundt seg", player.needs));
  overview.append(left, renderAttributes(attributes), right);
  ...
  [["season", "Sesong"], ["training", "Trening"], ["history", "Historikk"]].forEach(([id, label]) => {
    ...
    button.addEventListener("click", () => {
      ...
      renderProfile(playerId, { allowLocked });
    });
    tabs.append(button);
  });
...
export function openManagerPlayerProfile(playerId, { allowLocked = false } = {}) {
  if (!runtime || !playerId) return false;
  activePlayerId = String(playerId);
  currentProfileTab = "season";
  const dialog = ensureProfileDialog();
  if (!renderProfile(activePlayerId, { allowLocked })) return false;
  if (!dialog.open) dialog.showModal();
  return true;
}

function openPlayerProfile(playerId) {
  return openManagerPlayerProfile(playerId);
}
...
function enhanceLineupChoices() {
  ...
  document.querySelectorAll("#lineupPlayerChoices > .lineup-player-card:not([data-player-workspace-enhanced])").forEach((selectButton) => {
    ...
    const profileButton = node("button", "lineup-player-profile-link");
    profileButton.type = "button";
    profileButton.append(node("strong", "", name), node("span", "", positions));
    ...
    const isSelected = selectButton.classList.contains("is-selected");
    selectButton.replaceChildren(node("strong", "", isSelected ? "Valgt" : "Velg"), node("span", "", "Sett inn"));
    selectButton.setAttribute("aria-label", `${isSelected ? "Valgt" : "Sett inn"} ${name} på valgt plass`);
...
function installObservers() {
  const choices = document.getElementById("lineupPlayerChoices");
  if (choices) {
    new MutationObserver(() => requestAnimationFrame(enhanceLineupChoices)).observe(choices, { childList: true });
  }
  const gate = document.getElementById("squadSetupGate");
  if (gate) {
    new MutationObserver(() => requestAnimationFrame(() => renderCompactStatus(liveRows())))
      .observe(gate, { subtree: true, childList: true, characterData: true, attributes: true });
  }
  document.addEventListener("change", (event) => {
    if (event.target?.matches?.("#formationSelect, #tacticSelect")) scheduleRosterRender();
  });
  window.addEventListener("updateProfile", scheduleRosterRender);
  window.addEventListener("storage", scheduleRosterRender);
  window.addEventListener("hgfm:open-player-profile", (event) => {
    const playerId = event.detail?.playerId;
    if (playerId) openManagerPlayerProfile(playerId, { allowLocked: event.detail?.allowLocked !== false });
  });
}
