...
**Avdeling 1 (8 gjenstår, 2 av dem undersøkt):** Eik Tønsberg · Vidar · ~~Kvik Halden~~ · Sandviken · Lysekloster · Sotra · Træff · ~~Bjarg~~

**Kvik Halden er lest og stoppet på én avgjørelse (24.08.2026).** Kilden ga **44 dokumenterte A-lagsnavn** — tjue historiske fra klubbens egen årstallsliste og Wikipedias bildetekst av cupvinnerlaget 1918, og 24 fra A-lagstroppen 2023 — men bare **9 med presis posisjon**, og det kreves 15 spillbare. Spørsmålet som avgjør er nytt: **teller en kilde som sier «forsvar» som posisjon?** 2023-troppen oppgir `K`, `F`, `MB` og `A` — keeper, forsvar, midtbane, angrep. `K` er entydig `GK`; de tre andre er grupper. Skrives gruppen som gruppens posisjoner blir omtrent 25 spillbare og klubben lander; teller den som ingen posisjon, blir det 9 og klubben forblir `pending`. Motorens egen `SQUAD_GROUPS` er nettopp disse fire gruppene, men `naturalPositions` er et felt for posisjoner — og fire naturlige posisjoner påstår en allsidighet kilden ikke hevder. **Valget avgjør formen på alle de fjorten gjenstående klubbene**, og er derfor ikke tatt i importen. Se `docs/P2_KVIK_HALDEN_SOURCE_PASS.md`.

**Passet fant også to ting kildelista ikke hadde flagget.** Kvik Halden er en **sammenslåing fra 1997** (FK Kvik + Halden Fotballklubb), og cupgullet i 1918 ble tatt av FK Kvik — samme problem som Sotra, som listen flagget. Og navnekollisjonene traff: av seks historiske navn med Wikipedia-artikkel var **tre en annen person** — en skøyteløper, en komponist og en dansk ordfører. I tillegg motsier katalogen troppen på ett navn: `mathias_engebretsen` står som `GK`, mens Kvik-troppen fører ham som midtbane. Krysskoblingen skal ikke gjøres før det er avklart.
